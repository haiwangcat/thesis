...
Here let's look at the RCC (Reverse C Compiler) compiler developed by \cite{Carothers1999}.
...
The RCC relies on that all statements in the original program contain simple expressions, and the program is structured. 
...
Instead, thanks to the program analysis, in our method this state variable is only stored once.
In addition, RCC tries to recover every variable that is modified in the original program, which may lead to lots of unnecessary recoveries that is not related to state variables, but our method only cares about recovering state variables. 
Another advantage of our method comparing to RCC is that we can handle arbitrary control flows but RCC can only handle structured programs. 
This is because RCC relies on proper shapes of the original program to build the reverse program.



\paragraph{Value graph based approach} Akgul and Mooney introduced a more sophisticated incremental inversion algorithm that uses def-use analysis to invert some assignment statements that are not lossless \cite{Akgul2004a}; we refer to this approach as regenerative incremental inversion.
...
One technique to obtain the previous value of \texttt{a} is to re-execute its definition. For example, assume before \texttt{a} is modified, its previous definition is \texttt{a = b + c}, then we could retrieved its old value by redefining it using the same expression: \texttt{a = b + c}. 
Note that in OPDES, the variables we want to retrieve are normally the input of the program, which don't have ``previous'' definitions. 
Therefore, this technique only works on those local variables which are used to retrieved state variables.
Another technique is to examine all the uses of \texttt{a} and see if its value can be retrieved from any of its uses. 
...
Note that this technique only works for some special operations like plus and minus.
...
The control flows are handled based on the phi functions in SSA form \cite{Cytron1991}, but they cannot handle loops properly.
...
Further more, our method can take care of arbitrary control flows, including loops.
...
\item  Our compiler can make use of human knowledge and generate better results. We feed those high level informations to our compiler and in practice the generated result are improved. 
