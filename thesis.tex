...
As stack is a LIFO (last-in-first-out) data structure, a variable that is firstly saved and pushed will be popped at last. 
...
To amortize this overhead, we could use a memory pool that allocates memories to stored variables. In addition, we also observed that most stored variables are of basic types like integers and floating points.
...
Given an object $obj$ of type $class \;S$ in the original program, if it is stored and restored in the forward and reverse programs, we have to make sure the state is recovered  after running the reverse program with subject of that  before running the  forward program.
...
When we restore this object, normally the object is already there, and we can just assign the copy to it using the assignment operator. 
...
In the next part we will propose a new method to handle this drawback.
...
As a result, calling the copy constructor of the $Base$ class will not do the whole copy of that object, but only a part of it that belongs to the type $Base$ only.
...
\begin{center}
    ...
    \\ \hline
    vector::pop\_back() deque::pop\_back() list::pop\_back() & Store the popped element, and then call pop\_back() & Restore the saved element and call push\_back() to push it to the vector.
    \\ \hline
